...
namespace OnlineLU.Client.ConsoleService.AzureHelper
{
    public class TestFileCreator
    {
        ...
        public void CreateFilePrecision(int ordem, int precision)
        {
            Random rand = new Random();
        ...
        public void OpenFileAndSendToBlob(int range, int precisionChar, string container, bool showSummary, int nthreads)
        {
            ...
            if (File.Exists(CombinePathFile(range)))
            {
                ...
                {
                    using (PutBlobAsync _queue = new PutBlobAsync(ref measuring, nthreads, range, showSummary))
                    {
                        for (int i = 0; i < range; i++)
                        {
                            string _blobName = i.ToString() + ".zip";
                            var _blobFile = new BlobInfo(i, container, _blobName, range, precisionChar);
        ...
        public bool GetMatrixFromBlob(int range, int precisionChar, string container, ref float[] matrix, bool showSummary, int nthreads)
        {
            precisionChar += 3; //para o parser da string de precisão é necessário considerar os caracteres "0" "." e ";"
            MeasuringTime[] measuring = new MeasuringTime[range + 2];
            measuring[range] = new MeasuringTime(-1); //tempos de processamento da fila
            ...
            using (GetBlobAsync _queue = new GetBlobAsync(ref measuring, nthreads, range, ref matrix, showSummary))
            {
                for (int i = 0; i < range; i++)
                {
                    string _blobName = i.ToString() + ".zip";
                    var _blobFileDes = new BlobInfo(i, container, _blobName, range, precisionChar);
